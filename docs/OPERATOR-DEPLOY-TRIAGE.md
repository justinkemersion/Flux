...
| Backup-scheduler errors only in `flux-web` logs | SMTP ops alerts unset or send failed | Set `FLUX_ALERT_EMAIL_TO=justin@vsl-base.com` plus `FLUX_SMTP_*` or `FLUX_SMTP_URL` in `docker/web/.env`; recreate `flux-web`. Look for `ops-alert-email: send failed` |
