...
- Optional repo **`FLUX.md`** is the project brief (distinct from dashboard operator metadata). Generate a first draft with your coding tool, then **`flux project brief push`** to show it on the project page. **`flux project brief prompt`** prints a copyable generation prompt when the file is missing.
