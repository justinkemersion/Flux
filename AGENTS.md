...
- Put **`slug`** (and **`hash`** when required) in a repo-root **`flux.json`** so `flux push migrations/` or `flux push ./migration.sql` does not require `-p` / `--hash` every time (`flux push --help`). Directory push applies ordered `.sql` files with a `flux.flux_migrations` ledger (`flux` schema, not in PostgREST); use **`flux push --plan`**, **`--dry-run`**, and **`flux migrations list`** to inspect before applying. **`flux migrations`** is the SQL ledger—not **`flux migrate`** (v2_shared → v1_dedicated). **Do not edit a versioned migration after it has been applied—create a new migration instead.** Single files under `migrations/` default to versioned; elsewhere default to raw. Use **`flux push path.sql --mode repeatable --force`** for idempotent demo seed/reset scripts (ledger: `flux.flux_repeatable_scripts`). Use **`--mode raw`** for intentional ad-hoc runs under `migrations/`.
