...
export async function cmdProjectBrief(
  ...
  if (opts.generate) {
    ...
    if (opts.save) {
      ...
      console.log(
        chalk.dim(
          `To keep in git: save as FLUX.md in your repo, edit, then \`flux project brief push --hash ${hash}\`. Or \`flux project brief generate --save --hash ${hash}\` for dashboard-only.`,
        ),
      );
    }
    return;
